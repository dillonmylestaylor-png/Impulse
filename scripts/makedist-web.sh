...
if [ ! -d "$IPLUG2_ROOT" ]; then
  echo "Error: iPlug2 directory not found at $IPLUG2_ROOT" >&2
  echo "Please clone or install the iPlug2 dependency so this build can run." >&2
  exit 1
fi
...
if [ -z "${EMSDK:-}" ]; then
  if command -v emcc >/dev/null 2>&1; then
    ...
    if command -v realpath >/dev/null 2>&1; then
      EMCC_PATH="$(realpath "$EMCC_PATH")"
    elif command -v python3.14 >/dev/null 2>&1; then
      EMCC_PATH="$(python3.14 -c 'import pathlib, sys; print(pathlib.Path(sys.argv[1]).resolve())' "$EMCC_PATH")"
    elif command -v python3 >/dev/null 2>&1; then
      EMCC_PATH="$(python3 -c 'import pathlib, sys; print(pathlib.Path(sys.argv[1]).resolve())' "$EMCC_PATH")"
    fi
    EMSDK="$(dirname "$(dirname "$EMCC_PATH")")"
  fi
fi

if [ -z "${EMSDK:-}" ]; then
  echo "EMSDK is not set and emcc was not found in PATH" >&2
  exit 1
fi

if command -v python3.14 >/dev/null 2>&1; then
  PYTHON3="$(command -v python3.14)"
elif command -v python3 >/dev/null 2>&1; then
  PYTHON3="$(command -v python3)"
  if ! "$PYTHON3" -c 'import sys; sys.exit(0 if sys.version_info >= (3, 10) else 1)'; then
    echo "python3 must be 3.10 or greater; found $($PYTHON3 --version 2>&1)" >&2
    echo "Install python3.14 or set a compatible python3 in PATH." >&2
    exit 1
  fi
else
...
PYTHON_DIR="$(cd "$(dirname "$PYTHON3")" >/dev/null 2>&1 && pwd)"
export PATH="$PYTHON_DIR:$PATH"
export EMSDK_PYTHON="$PYTHON3"
export EM_LLVM_ROOT="$EMSDK/libexec/llvm/bin"
export PATH="$EM_LLVM_ROOT:/opt/homebrew/opt/llvm/bin:$PATH"
export PYTHON="$PYTHON3"

FILE_PACKAGER="$EMSDK/libexec/tools/file_packager.py"
EMRUN="$PYTHON3"
EMRUN_SCRIPT="$IPLUG2_ROOT/Scripts/emrun/emrun.py"
...
if [ "$(ls -A ./resources/img/*@2x*.png)" ]; then
  ...
  "$PYTHON3" "$FILE_PACKAGER" imgs@2x.data --use-preload-plugins --preload ./build-web/2x/ --use-preload-cache --indexedDB-name="/$PROJECT_NAME_pkg" --exclude *DS_Store >> ./imgs@2x.js
  rm -r ./build-web/2x
fi
...
if [ "$BUILD_DSP" -eq "1" ]; then
  ...
  EMSDK_PYTHON="$PYTHON3" emmake make --makefile "$PROJECT_NAME-wam-processor.mk"
...
EMSDK_PYTHON="$PYTHON3" emmake make --makefile "$PROJECT_NAME-wam-controller.mk" EXTRA_CFLAGS=-DWEBSOCKET_CLIENT=$WEBSOCKET_MODE
...
if [ "$LAUNCH_EMRUN" -eq "1" ]; then
  ...
  if [ "$EMRUN_CONTAINER" -eq "1" ]; then
    "$EMRUN" "$EMRUN_SCRIPT" --no_browser --serve_after_close --serve_after_exit --port=$EMRUN_SERVER_PORT --hostname=0.0.0.0 .
  elif [ "$EMRUN_SERVER" -eq "0" ]; then
    "$EMRUN" "$EMRUN_SCRIPT" --browser "$EMRUN_BROWSER" --no_server --port=$EMRUN_SERVER_PORT index.html
  else
    "$EMRUN" "$EMRUN_SCRIPT" --browser "$EMRUN_BROWSER" --no_emrun_detect index.html
  fi
else
