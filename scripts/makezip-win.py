...
def main():
    ...
    if not zip:
        installer = "\\build-win\\installer\\Impulse Installer.exe"

        if demo:
            installer = "\\build-win\\installer\\Impulse Demo Installer.exe"

        files = [
            projectpath + installer,
            projectpath + "\\installer\\changelog.txt",
            projectpath + "\\installer\\known-issues.txt",
            projectpath + "\\manual\\Impulse manual.pdf",
        ]
    else:
        files = [
            projectpath
            + "\\build-win\\Impulse.vst3\\Contents\\x86_64-win\\Impulse.vst3",
            projectpath + "\\build-win\\Impulse_x64.exe",
        ]
    ...
    files = [
        projectpath + "\\build-win\\pdbs\\Impulse-vst3_x64.pdb",
        projectpath + "\\build-win\\pdbs\\Impulse-app_x64.pdb",
    ]
