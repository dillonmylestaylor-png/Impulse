...
def main():
    ...
    captures_src = projectpath + "/captures/"
    if os.path.exists(captures_src):
        captures_dst = os.path.join(dst, "captures")
        if not os.path.exists(captures_dst):
            os.makedirs(captures_dst, 0o0755)
        for f in os.listdir(captures_src):
            if f.endswith(".nam"):
                print("copying " + f + " to " + captures_dst)
                shutil.copy(os.path.join(captures_src, f), captures_dst)
