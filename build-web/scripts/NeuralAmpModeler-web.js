...
function findWasmBinary() {
  return locateFile('NeuralAmpModeler-web.wasm');
}
...
var ASM_CONSTS = {
  155484: () => { if(Module.syncdone == 1) { Module.syncdone = 0; FS.syncfs(false, function (err) { assert(!err); console.log("Synced to IDBFS..."); Module.syncdone = 1; }); } },  
 155646: ($0) => { var name = '/' + UTF8ToString($0) + '_data'; FS.mkdir(name); FS.mount(IDBFS, {}, name); Module.syncdone = 0; FS.syncfs(true, function (err) { assert(!err); console.log("Synced from IDBFS..."); Module.syncdone = 1; ccall('iplug_fsready', 'v'); }); },  
 155893: ($0) => { var pGraphics = $0; var canvas = Module.canvas; if (!canvas) return; canvas._iplugGraphics = pGraphics; canvas.addEventListener('mousedown', function(e) { Module._iGraphicsMouseCallback(pGraphics, 0, e.offsetX, e.offsetY, e.movementX, e.movementY, e.buttons, e.button, e.shiftKey, e.ctrlKey, e.altKey); }); canvas.addEventListener('mouseup', function(e) { Module._iGraphicsMouseCallback(pGraphics, 1, e.offsetX, e.offsetY, e.movementX, e.movementY, e.buttons, e.button, e.shiftKey, e.ctrlKey, e.altKey); }); canvas.addEventListener('mousemove', function(e) { Module._iGraphicsMouseCallback(pGraphics, 2, e.offsetX, e.offsetY, e.movementX, e.movementY, e.buttons, e.button, e.shiftKey, e.ctrlKey, e.altKey); }); canvas.addEventListener('mouseenter', function(e) { Module._iGraphicsMouseCallback(pGraphics, 3, e.offsetX, e.offsetY, e.movementX, e.movementY, e.buttons, e.button, e.shiftKey, e.ctrlKey, e.altKey); }); canvas.addEventListener('mouseleave', function(e) { Module._iGraphicsMouseCallback(pGraphics, 4, e.offsetX, e.offsetY, e.movementX, e.movementY, e.buttons, e.button, e.shiftKey, e.ctrlKey, e.altKey); }); canvas.addEventListener('wheel', function(e) { Module._iGraphicsWheelCallback(pGraphics, e.offsetX, e.offsetY, e.deltaY, e.shiftKey, e.ctrlKey, e.altKey); e.preventDefault(); }, { passive: false }); },  
 157215: ($0, $1, $2) => { if(typeof window[Module.UTF8ToString($0)] === 'undefined' ) { console.log("warning - SAMFUI called before controller exists"); } else { window[Module.UTF8ToString($0)].sendMessage('SAMFUI', "", Module.HEAPU8.slice($1, $1 + $2).buffer); } },  
 157457: ($0) => { if(typeof window[Module.UTF8ToString($0)] === 'undefined' ) { console.log("warning - SendDSPIdleTick called before controller exists"); } else { window[Module.UTF8ToString($0)].sendMessage("TICK", "", 0.); } }
};
function createWebGLContextForShadowDOM() { var canvas = Module.canvas; if (!canvas) return 0; var attrs = { stencil: true, depth: true, antialias: true, alpha: true }; var ctx = canvas.getContext("webgl", attrs) || canvas.getContext("experimental-webgl", attrs); if (!ctx) return 0; return GL.registerContext(ctx, attrs); }
...
var ___getTypeName,
  ...
  _iplug_fsready,
  _main,
  _iGraphicsMouseCallback,
  _iGraphicsWheelCallback,
  __ZN3WAM9Processor4initEjjPv,
  _wam_init,
  _wam_terminate,
  _wam_resize,
  _wam_onparam,
...
function assignWasmExports(wasmExports) {
  ...
  _iplug_syncfs = Module['_iplug_syncfs'] = wasmExports['iplug_syncfs'];
  _iplug_fsready = Module['_iplug_fsready'] = wasmExports['iplug_fsready'];
  _main = Module['_main'] = wasmExports['main'];
  _iGraphicsMouseCallback = Module['_iGraphicsMouseCallback'] = wasmExports['iGraphicsMouseCallback'];
  _iGraphicsWheelCallback = Module['_iGraphicsWheelCallback'] = wasmExports['iGraphicsWheelCallback'];
  __ZN3WAM9Processor4initEjjPv = Module['__ZN3WAM9Processor4initEjjPv'] = wasmExports['_ZN3WAM9Processor4initEjjPv'];
  _wam_init = Module['_wam_init'] = wasmExports['wam_init'];
  _wam_terminate = Module['_wam_terminate'] = wasmExports['wam_terminate'];
  _wam_resize = Module['_wam_resize'] = wasmExports['wam_resize'];
  _wam_onparam = Module['_wam_onparam'] = wasmExports['wam_onparam'];
  _wam_onmidi = Module['_wam_onmidi'] = wasmExports['wam_onmidi'];
  _wam_onsysex = Module['_wam_onsysex'] = wasmExports['wam_onsysex'];
  _wam_onprocess = Module['_wam_onprocess'] = wasmExports['wam_onprocess'];
  _wam_onpatch = Module['_wam_onpatch'] = wasmExports['wam_onpatch'];
  _wam_onmessageN = Module['_wam_onmessageN'] = wasmExports['wam_onmessageN'];
  _wam_onmessageS = Module['_wam_onmessageS'] = wasmExports['wam_onmessageS'];
  _wam_onmessageA = Module['_wam_onmessageA'] = wasmExports['wam_onmessageA'];
...
var wasmImports = {
  ...
  /** @export */
  createWebGLContextForShadowDOM,
  /** @export */
